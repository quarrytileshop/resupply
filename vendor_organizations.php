<?php
// vendor_organizations.php – Professional My Customers hub with Invite/Prefill – 2026-05-12
...
// Handle adding new organization (pending)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add_org') {
    $name = trim($_POST['name'] ?? '');
    $account_number = trim($_POST['account_number'] ?? '');
    if ($name) {
        $stmt = $pdo->prepare("INSERT INTO organizations (name, account_number, vendor_id, approval_status) VALUES (:name, :account_number, :vendor_id, 'pending')");
        $stmt->execute(['name' => $name, 'account_number' => $account_number, 'vendor_id' => $vendor_id]);
        $success = "Organization added as pending. Approve it below.";
    }
}
...
if (isset($_GET['approve'])) {
    $id = (int)$_GET['approve'];
    $pdo->prepare("UPDATE organizations SET approval_status = 'approved' WHERE id = :id AND vendor_id = :vendor_id")->execute(['id' => $id, 'vendor_id' => $vendor_id]);
    header("Location: vendor_organizations.php?tab=pending&msg=approved");
    exit;
}
if (isset($_GET['delete'])) {
    $id = (int)$_GET['delete'];
    $pdo->prepare("DELETE FROM organizations WHERE id = :id AND vendor_id = :vendor_id")->execute(['id' => $id, 'vendor_id' => $vendor_id]);
...
<div class="container mt-4">
    ...
    <p class="text-muted">Manage organizations under your vendor account. Pending ones must be approved before they can register users.</p>

    <?php if (isset($_GET['msg'])): ?>
        <div class="alert alert-success"><?= $_GET['msg'] === 'approved' ? 'Organization approved!' : 'Organization deleted.' ?></div>
    <?php endif; ?>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <?php endif; ?>
                                <a href="vendor_invite_org.php?org_id=<?= $org['id'] ?>" class="btn btn-info btn-sm">Invite / Prefill Registration</a>
                                <a href="?delete=<?= $org['id'] ?>" onclick="return confirm('Delete this organization?')" class="btn btn-danger btn-sm">Delete</a>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
